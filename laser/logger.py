...
import laser.metrics_ogb as metrics_ogb
from laser.metric_wrapper import MetricWrapper
...
class CustomLogger(Logger):
    ...
    def classification_multilabel(self):
        ...
        #                     threshold=0.,
        #                     cast_to_int=True)
        acc = torch.argmax(true, dim=1).eq(pred_score.max(1)[1]).to(torch.float).mean()
    ...
    def update_stats(self, true, pred, loss, lr, time_used, params,
        ...
        if dataset_name == 'ogbg-code2':
            ...
            from laser.loader.ogbg_code2_utils import idx2vocab, \
                decode_arr_to_seq
