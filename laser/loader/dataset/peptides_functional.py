...
class PeptidesFunctionalDataset(InMemoryDataset):
    def __init__(self, root='datasets', smiles2graph=smiles2graph,
        ...
        self.data, self.slices = torch.load(self.processed_paths[0], weights_only=False) # weights_only=False necessary for MPS
    ...
    def processed_dir(self) -> str:
        ...
        name += metadata
        return osp.join(self.folder, name)
    ...
    def process(self):
        ...
        if self.pre_transform is not None:
            data_list = Parallel(n_jobs=-1, batch_size='auto')(
                delayed(self.pre_transform)(s) for s in tqdm(data_list)
            )
